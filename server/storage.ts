...
export interface IStorage {
  ...
  fixAlistPaths(): Promise<{ updated: number; errors: string[] }>;
...
export class MemStorage implements IStorage {
  ...
  async fixAlistPaths(): Promise<{ updated: number; errors: string[] }> {
    let updated = 0;
    const errors: string[] = [];

    const entries = Array.from(this.content.entries());
    for (const [id, item] of entries) {
      if (item.driveLink && item.driveLink.startsWith('/GD01/') && !item.driveLink.startsWith('/GD01/MOVIE/')) {
        const newPath = item.driveLink.replace('/GD01/', '/GD01/MOVIE/');
        item.driveLink = newPath;
        this.content.set(id, item);
        updated++;
      }
    }
...
export class DbStorage implements IStorage {
  ...
  async fixAlistPaths(): Promise<{ updated: number; errors: string[] }> {
    ...
    try {
      // Find all content items with driveLink starting with /GD01/
      const items = await ContentItemModel.find({
        driveLink: { $regex: '^/GD01/' }
      });

      let updated = 0;
      for (const item of items) {
        // Skip if already has /GD01/MOVIE/
        if (item.driveLink.startsWith('/GD01/MOVIE/')) {
          continue;
        }

        try {
          const newPath = item.driveLink.replace('/GD01/', '/GD01/MOVIE/');
          await ContentItemModel.updateOne(
            { _id: item._id },
            { $set: { driveLink: newPath } }
          );
          updated++;
          console.log(`✅ Fixed path for ${item.title}: ${item.driveLink} → ${newPath}`);
        } catch (err: any) {
          errors.push(`Failed to update ${item.title}: ${err.message}`);
        }
      }
